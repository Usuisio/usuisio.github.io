---
title: ぬくもりロボット工房
description: あたたかみのあるゲームをつくる個人開発の工房「ぬくもりロボット工房」の公式サイト。ブラウザで遊べる無料ゲームやSteam作品を公開しています。
params:
  hero_title: '<span class="wavy">あたたかみ</span>のあるゲームを、<br>ちいさな工房でつくっています。'
  hero_sub: ぬくもりロボット工房は、個人でゲームをつくっている工房です。ブラウザですぐ遊べる小さなゲームから、Steamで販売中のアドベンチャーまで。
  mascot_alt: ぬくもりロボット工房のマスコット。作業着を着てスパナをかついだ白うさぎ
  games_title: つくったゲーム
  games:
    - title: 夏の宿題チキンレース！
      blurb: 夏休みの宿題をどれだけギリギリまで放置して、それでも8/31までに完成させられるかを競うリアルタイムのチキンレース。約3分・「放置した日数」ランキングつき。
      url: /natsuyasumi/
      img: /img/natsuyasumi.webp
      alt: 夏の宿題チキンレース！のタイトル画面。畳の部屋の白い紙にロゴとスイカ
      tags: [ブラウザ, 無料, 新作]
    - title: 通せ！連載会議
      blurb: 白紙の原稿を抱えた漫画家となり、編集エドと次回作をでっち上げて、編集長ヨルに連載をプレゼンするアドベンチャー。約3分・unity1week「しろ」参加作品。
      url: /rensai/
      img: /img/rensai.webp
      alt: 通せ！連載会議のタイトル画面。喫茶店のテーブルに置かれた原稿用紙とラフスケッチ
      tags: [ブラウザ, 無料]
    - title: 人類をダメにするマザーAI
      blurb: 5×5の箱庭にタイルを置いて、人類をとことん甘やかして「しあわせ❤」を集める管理シミュレーション。25日制・1プレイ約30分。
      url: /hitodame/
      img: /hitodame/title-ja.webp
      alt: 人類をダメにするマザーAIのタイトル画面。紙の上のロゴとマザーAIの立ち絵
      tags: [ブラウザ, 無料]
    - title: リペア・ザ・ココロ
      blurb: こわれた子守ロボットを、ブラウザの開発者コンソール（F12）に本物のコマンドを打って修理するテキストアドベンチャー。約15分・マルチエンディング。
      url: /repair-the-cocoro/
      img: /cocoro/scene-ja.webp
      alt: リペア・ザ・ココロの会話シーン。子守ロボット・ロコロが嬉しそうに話している
      tags: [ブラウザ, 無料]
    - title: DoppelTest（ドッペルテスト）
      blurb: 社会になじめない子カガリと、その身代わりを務めるロボットカガミ。ふたりの14日間を見守るアドベンチャー。
      url: /doppeltest/
      img: /img/doppeltest.webp
      alt: DoppelTestのキービジュアル。カガリとロボットのカガミが向かい合って立っている
      tags: [Steam, ￥580]
    - title: ハッキング・ポクラッテ！
      blurb: 悪の天才ハッカーになってお屋敷に潜入し、メイドロボットをハッキングしてお宝のありかを聞き出すアドベンチャー。約3分。
      url: /poklatte/
      img: /img/poklatte.webp
      alt: ハッキング・ポクラッテ！のメイドロボ、ポクラッテのアップ
      tags: [ブラウザ, 無料]
  more_label: unityroomの作品一覧はこちら
  more_url: https://unityroom.com/users/usuisio
  gallery_label: ギャラリー ── イラストやラフのまとめ
  gallery_url: /gallery/
  org_name: ぬくもりロボット工房
  org_alt: Nukumori Robot Workshop
  sameas: ["https://x.com/ususio0unity", "https://www.youtube.com/@user-NukuRobo", "https://unityroom.com/users/usuisio", "https://usuisio.itch.io"]
  about_label: 工房について
  about_url: /about/
  footer_links:
    - { label: X（Twitter）, url: "https://x.com/ususio0unity" }
    - { label: YouTube, url: "https://www.youtube.com/@user-NukuRobo" }
    - { label: unityroom, url: "https://unityroom.com/users/usuisio" }
    - { label: itch.io, url: "https://usuisio.itch.io" }
    - { label: Steam, url: "https://store.steampowered.com/app/3849320/" }
  copyright: © ぬくもりロボット工房
---
